...
window.regionScoreboard = function() {
  ...
  var dlg = dialog({title:'区域分数',html:'加载区域分数...',width:450,minHeight:320});
...
function regionScoreboardScoreHistoryChart(result, logscale) {
  ...
  var svg = '<div><svg width="400" height="130">'
           ...
           +paths
           +otherSvg.join('')
           +'<foreignObject height="18" width="45" y="111" x="0" class="node"><label title="对数刻度">'
...
function regionScoreboardScoreHistoryTable(result) {
  ...
  var table = '<table class="checkpoint_table"><thead><tr><th>检查点</th><th>启蒙军</th><th>反抗军</th></tr></thead>';
...
function regionScoreboardSuccess(data,dlg,logscale) {
  ...
  var agentTable = '<table><tr><th>#</th><th>特工</th></tr>';
  ...
  $('g.checkpoint', dlg).each(function(i, elem) {
    ...
    var tooltip = '检查点:\t'+elem.attr('data-cp')
      + '\n启蒙军:\t' + digits(elem.attr('data-enl'))
      + '\n反抗军:\t' + digits(elem.attr('data-res'));
    elem.tooltip({
