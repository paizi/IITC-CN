...
function createDefaultBaseMapLayers() {
  ...
  baseLayers['Google 默认Ingress地图'] = L.gridLayer.googleMutant(
    { type:'roadmap',
      maxZoom: 21,
  ...
  baseLayers['Google 道路地图'] = L.gridLayer.googleMutant({type:'roadmap', maxZoom: 21});
  var trafficMutant = L.gridLayer.googleMutant({type:'roadmap', maxZoom: 21});
  trafficMutant.addGoogleLayer('TrafficLayer');
  baseLayers['Google 道路+交通地图'] = trafficMutant;
  baseLayers['Google 卫星地图'] = L.gridLayer.googleMutant({type:'satellite', maxZoom: 21});
  baseLayers['Google 卫星+道路地图'] = L.gridLayer.googleMutant({type:'hybrid', maxZoom: 21});
  baseLayers['Google 地形图'] = L.gridLayer.googleMutant({type:'terrain', maxZoom: 21});
...
//    zoomAnimation: false,
  ...
  if (PLAYER.team == 'RESISTANCE') {
    addLayers['反抗军'] = factionLayers[TEAM_RES];
    addLayers['启蒙军'] = factionLayers[TEAM_ENL];
  } else {
    addLayers['启蒙军'] = factionLayers[TEAM_ENL];
    addLayers['反抗军'] = factionLayers[TEAM_RES];
  }
  if (!isLayerGroupDisplayed('反抗军', true)) hiddenLayer.push (factionLayers[TEAM_RES]);
  if (!isLayerGroupDisplayed('启蒙军', true)) hiddenLayer.push (factionLayers[TEAM_ENL]);

  setFactionLayersState (TEAM_NONE, true);
  setFactionLayersState (TEAM_RES, isLayerGroupDisplayed('反抗军', true));
  setFactionLayersState (TEAM_ENL, isLayerGroupDisplayed('启蒙军', true));
  ...
  $.each(hiddenLayer, function(ind, layer){
    ...
    $('#portaldetails').html('<div class="layer_off_warning">'
                            +'<p><b>警告</b>: 部分标准图层被关闭。 部分 portal/links/fields 将不会显示。</p>'
                            +'<a id="enable_standard_layers">启用标准图层</a>'
                            +'</div>');
...
window.setupPlayerStat = function() {
  ...
  var t = '等级:\t' + level + '\n'
        ...
        + 'AP:\t' + digits(ap) + '\n'
        + (nextLvlAp > 0 ? '距下次升级还差:\t' + lvlUpAp + ' AP' : '已达到最高等级(!)')
        + '\n\邀请函:\t'+PLAYER.available_invites
        + '\n\n注意：您的玩家状态只会在刷新网页时更新 (F5)';
  ...
  $('#playerstat').html(''
    ...
    + '<div id="stats">'
    + '<sup>XM: '+xmRatio+'%</sup>'
    + '<sub>' + (nextLvlAp > 0 ? '等级: '+lvlApProg+'%' : '最高等级') + '</sub>'
    + '</div>'
    + '</h2>'
  );
}
...
function boot() {
  ...
  if(window.bootPlugins) {
    ...
    if (Object.keys(badPlugins).length > 0) {
      ...
      warning += '</ul><p>请卸载有问题的插件并重载页面。 查看 <a href="https://iitc.modos189.ru/faq.html">FAQ</a> 获取帮助。</p><p><i>提示: 对于IITC来说，安全的禁用问题插件是很麻烦的。</i></p>';
