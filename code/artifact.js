...
window.artifact.setup = function() {
  ...
  $('#toolbox').append(' <a onclick="window.artifact.showArtifactList()" title="显示神器清单">神器</a>');
...
window.artifact.showArtifactList = function() {
  ...
  $.each(artifact.artifactTypes, function(type,type2) {
    ...
    html += '<tr><th>Portal</th><th>细节</th></tr>';
      ...
      if (type in data) {
        ...
        if (data[type].target !== undefined) {
          if (data[type].target == TEAM_NONE) {
            row += '<span class="target">目标Portal</span> ';
          } else {
...
//          row += '<span class="fragments'+(data[type].target?' '+TEAM_TO_CSS[data[type].target]:'')+'">'+fragmentName+': #'+data[type].fragments.join(', #')+'</span> ';
          row += '<span class="碎片'+(data[type].target?' '+TEAM_TO_CSS[data[type].target]:'')+'">'+fragmentName+': yes</span> ';
        }
    ...
    if (tableRows.length == 0) {
      html += '<tr><td colspan="2"><i>这个时段没有神器</i></td></tr>';
    }
  ...
  html += "<hr />"
        + "<p>在2015年夏天，Niantic修改了神器能量塔的数据格式，我们已经无法得知:</p>"
        + "<ul><li>目标能量塔属于哪支阵营 - 只知道它是一个目标能量塔</li>"
        + "<li>碎片在哪个能量塔，只知道它有一个或多个碎片</li></ul>"
        + "<p>你可以选择一个能量塔，详细数据里包括碎片号码列表，但依然没有"
        + " 该目标更多的资料。</p>";
